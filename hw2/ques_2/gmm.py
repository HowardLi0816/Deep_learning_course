...
def m_step(x_all, rk_all):
    ...
    for i in range(k):
        ...
        down = rk_all[:, i].sum()
        for n in range(N):
            up += rk_all[n][i] * x_all[n]
        mean_tmp = up/down
        meank_new[i] = mean_tmp
        #print('aaaaa',meank_new[i])

        up_1 = np.zeros((d, d))
        for n in range(N):
            xn_uk = x_all[n] - mean_tmp
            #print(xn_uk)
...
def gmm(data, thresh, init_rk):
    ...
    #print(loss)
    iter = 0
    acc = eval(data, w, mean, cov, plot = True)
...
def eval(data, w, mean, cov, plot = False):
    N = data.shape[0]
    data_fea = data[:, :-1]
    data_label = data[:, -1]
    rk = e_step(data_fea, w, mean, cov)
...
if __name__ == '__main__':
    ...
    #eval
    print('Start eval!')
    acc = eval(data_np, w, mean, cov, plot = True)
    print('Final acc:', acc)